            ...
            <div className="results-section">
                <div className="result-and-title">
                    ...
                    <div className="result-box" id="main-details">
                        <div className="metric"><span title='The difference between the time reported by the like an NTP server and your local clock'>Offset</span><span>{data?.offset ? `${data.offset} ms` : 'N/A'}</span></div>
                        <div className="metric"><span title='The total time taken for a request to travel from the client to the server and back.'>Round-trip time</span><span>{data?.RTT ? `${data.RTT} ms` : 'N/A'}</span></div>
                        <div className="metric"><span title='The variability in delay times between successive NTP messages, calculated as std. dev. of offsets'>Jitter</span><span>{data?.jitter ? `${data.jitter} ms` : 'N/A'}</span></div>
    ...
    <div className="tooltip-text">
       RIPE Atlas tries to choose a probe near the user to perform more accurate measurements.
                    ...
                    <div className="result-box" id="ripe-details">
                        <div className="metric"><span title='The difference between the time reported by the like an NTP server and your local clock'>Offset</span><span>{ripeData?.measurementData.offset ? `${ripeData.measurementData.offset} ms` : 'N/A'}</span></div>
                        <div className="metric"><span title='The total time taken for a request to travel from the client to the server and back.'>Round-trip time</span><span>{ripeData?.measurementData.RTT ? `${ripeData.measurementData.RTT} ms` : 'N/A'}</span></div>
                        <div className="metric"><span title='The variability in delay times between successive NTP messages, calculated as std. dev. of offsets'>Jitter</span><span>{ripeData?.measurementData.jitter ? `${ripeData.measurementData.jitter} ms` : 'N/A'}</span></div>
